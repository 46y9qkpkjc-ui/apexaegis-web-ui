...
/* ─── Demo data — SSE continuous control monitoring ───────────────────── */
...
const complianceCategories: ComplianceCategory[] = [
  {
    ...
    checks: [
      ...
      { id: 'IAM-03', title: 'Device mTLS Enrollment', description: 'Managed desktop and mobile clients should authenticate to the device API with issued mTLS certificates.', category: 'identity', severity: 'critical', status: 'pass', finding: 'Device certificate enrollment is enabled with tenant-bound mTLS validation for desktop REST runtime APIs.', remediation: 'N/A', reference: 'NIST IA-3 / NIST SC-23', lastChecked: '2026-03-14T08:30:00Z' },
      { id: 'IAM-04', title: 'SCIM Provisioning', description: 'SCIM auto-provisioning should be enabled for users and groups from the primary IdP.', category: 'identity', severity: 'high', status: 'pass', finding: 'SCIM enabled on Okta with user and group mapping for admin, user portal, and desktop SSO workflows.', remediation: 'N/A', reference: 'SOC 2 CC6.2', lastChecked: '2026-03-14T08:30:00Z' },
  ...
  },
  {
    ...
    checks: [
      ...
      { id: 'ENC-05', title: 'mTLS Endpoint Separation', description: 'Gateway gRPC and desktop runtime APIs should use dedicated mTLS hostnames with separate trust stores.', category: 'encryption', severity: 'high', status: 'pass', finding: 'api.apexaegis.app uses JWT/OIDC for Web UI REST, device-api.apexaegis.app uses device mTLS, and gateway-api.apexaegis.app uses gateway gRPC mTLS.', remediation: 'N/A', reference: 'NIST SC-23', lastChecked: '2026-03-14T08:30:00Z' },
  ...
  },
  {
    ...
    checks: [
      { id: 'NET-01', title: 'Gateway Health & Availability', description: 'Internet access and private access gateways should publish heartbeat and health status to the management plane.', category: 'network', severity: 'high', status: 'pass', finding: 'Gateway health checks and heartbeat status are monitored for tunnel, SWG, DNS, CASB, and private access readiness.', remediation: 'N/A', reference: 'SOC 2 A1.2', lastChecked: '2026-03-14T08:30:00Z' },
      { id: 'NET-02', title: 'QUIC With TCP 443 Fallback', description: 'Client tunnels should use QUIC over UDP 443 with TCP 443 fallback for restrictive networks.', category: 'network', severity: 'medium', status: 'pass', finding: 'Desktop client supports QUIC on 443 and TLS/TCP fallback on 443 for networks that block UDP.', remediation: 'N/A', reference: 'NIST SC-8 / Zero Trust Architecture', lastChecked: '2026-03-14T08:30:00Z' },
      { id: 'NET-03', title: 'DNS Filtering', description: 'DNS resolver steering should be enforced when DNS filtering is enabled by policy or client configuration.', category: 'network', severity: 'high', status: 'pass', finding: 'DNS filtering policy supports malware, phishing, C2, newly registered domains, and suspicious application categories.', remediation: 'N/A', reference: 'NIST SC-20', lastChecked: '2026-03-14T08:30:00Z' },
      { id: 'NET-04', title: 'Private Access Steering', description: 'Private application routes should be delivered through tenant-aware policy and protected by device mTLS.', category: 'network', severity: 'high', status: 'pass', finding: 'Private access route policies are separated from SWG policy and fetched by the desktop client through the device mTLS API.', remediation: 'N/A', reference: 'NIST AC-4 / Zero Trust Architecture', lastChecked: '2026-03-14T08:30:00Z' },
      { id: 'NET-05', title: 'Gateway Port Exposure', description: 'Gateway and management APIs should expose only HTTPS/QUIC-compatible port 443 entry points.', category: 'network', severity: 'medium', status: 'pass', finding: 'Public control-plane and client-facing endpoints are consolidated on 443 across Web UI REST, device REST mTLS, gateway gRPC mTLS, QUIC, and TCP fallback.', remediation: 'N/A', reference: 'CIS 9.1', lastChecked: '2026-03-14T08:30:00Z' },
    ],
  },
  {
    ...
    checks: [
      ...
      { id: 'POL-04', title: 'ATP and SWG Inspection', description: 'Internet access allow rules should attach ATP, SWG, URL filtering, and SSL inspection profiles where applicable.', category: 'policy', severity: 'critical', status: 'pass', finding: 'ATP, SWG, URL filtering, and SSL inspection profiles are attached to internet access policy rules.', remediation: 'N/A', reference: 'NIST SI-3 / CIS 8.1', lastChecked: '2026-03-14T08:30:00Z' },
      { id: 'POL-05', title: 'CASB and DLP Controls', description: 'Sanctioned SaaS traffic should be inspected for app risk, tenant risk, and sensitive data leakage.', category: 'policy', severity: 'high', status: 'pass', finding: 'CASB application controls and DLP inspection are enabled for sanctioned SaaS and AI application categories.', remediation: 'N/A', reference: 'NIST SI-8 / CIS 9.3', lastChecked: '2026-03-14T08:30:00Z' },
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Continuous SSE control monitoring for internet access, private access, identity, and data protection</p>
          </div>
        </div>
